package xyz.segurapass.backend.exceptions.enums;
...
public enum ErrorEnum {
    ...
    USER_VERIFICATION_NOT_EXISTS(HttpStatus.CONFLICT, "Could not verify user email"),
    EMAIL_VERIFICATION_OFF(HttpStatus.CONFLICT, "Email verification is not enabled on this server"),
    LOGIN_INFORMATION_INCORRECT(HttpStatus.CONFLICT, "Email and/or password are incorrect"),
    EMAIL_PENDING_VERIFICATION(HttpStatus.CONFLICT, "Email is pending verification"),
    ...
    CREDENTIAL_COUNT_ABOVE_LIMIT(HttpStatus.NOT_ACCEPTABLE, "Credential count cannot be above limit"),
    CREDENTIAL_COUNT_BELOW_ZERO(HttpStatus.NOT_ACCEPTABLE, "Credential count cannot be below zero"),
    ...
    PASSWORD_INCORRECT(HttpStatus.CONFLICT, "Password is incorrect");
